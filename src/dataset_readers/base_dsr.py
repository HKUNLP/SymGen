...
def _encode_field(example, idx, **kwargs):
    ...
    tokenized_inputs = tokenizer.encode_plus(text, truncation=False, return_tensors='pt')
    return {
...
def encode_field(tokenizer, dataset_wrapper, field):
...
class BaseDatasetReader(torch.utils.data.Dataset):
    ...
    def init_dataset(self, task_name, field, dataset_path, dataset_split, ds_size):
        self.dataset_wrapper = get_dataset_wrapper(task_name,
                                                   ...
                                                   ds_size=ds_size)
        self.encoded_dataset = encode_field(self.tokenizer, self.dataset_wrapper, field)
