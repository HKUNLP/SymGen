...
class EvaluateTool(object):
    ...
    def evaluate(self, preds, golds):
        ...
        eval_results = parallel_run(eval_lfem, initializer=set_global_object,
                                    args_list=list(zip(questions, preds, golds, mrange)))
        lf_em = sum(eval_results) / len(golds)

        eval_results = parallel_run(eval_normem, args_list=list(zip(questions, preds, golds, mrange)))
        norm_em = sum(eval_results) / len(golds)
        metrics = {"lf_em": lf_em, "norm_em": norm_em}
        return metrics
